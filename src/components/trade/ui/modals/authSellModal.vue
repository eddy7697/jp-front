    ...
    <div class="authSellModal">
        <div class="content">
            ...
            <div class="container">
                ...
                <div class="row input-area">
                    <input class="col-8" v-model="authSellPrice" @keypress="onlyForCurrency" type="number" id="authSellValue" value="" placeholder="小數後兩位"/>
                    <label class="col-4" >出價</label>
                </div>
                <div class="row input-area">
                    <input class="col-8" v-model="authSellValue" onkeypress="return event.charCode >= 48 && event.charCode <= 57" type="number" id="authSellValue" value="" placeholder="單位最小為1"/>
                ...
                <p class="text-white text-center">*交易成功將會收取2%手續費用</p>
                <p class="text-white text-center">*成功上架後7日到期，系統自動下架</p>
            </div>
            <div class="container">
                <div class="row">
    ...
    export default {
        ...
        methods: {
            onlyForCurrency($event) {
                if(this.authSellPrice!=null && this.authSellPrice.indexOf(".")>-1 && (this.authSellPrice.split('.')[1].length > 1)){
                ...
                this.$store.dispatch('getApi', data).then((item)=>{
                    if (item['result'] == 'error') {
                        ...
                        item['data']['items'].forEach((obj)=> {
                            if(obj['features'][0]['code'] == 'trade_expenses') {
                                this.apiId = obj.id
                            }
                        })
            ...
            closeModal() {
                this.$store.dispatch('authSellModal', false)
            },
            sellCoin() {
                if(!this.apiId) {
                    swal.fire({
                ...
                data['features[0][code]'] = 'trade_expenses'
                data['features[0][handle][point]'] = Number(this.authSellValue*100)
                data['features[0][handle][price]'] = Number(this.authSellPrice)
                console.log(data)
                ...
                this.$store.dispatch('getApi', data).then((item)=>{
                    if (item['result'] == 'error') {
                        ...
                        if(item['data'].length > 0) {
                            ...
                            this.maxSellPrice = this.currency*1.1
                            console.log(this.maxSellPrice)
                        }
                    }
                }) 
            }
        ...
        computed:{
            compareResult() {
                return this.$store.state.memberInfo.userInfo.ucoin/this.authSellPrice
            },
            calUcoin() {
                if(this.authSellValue > this.compareResult) {
                    swal.fire({
                        title: 'Error!',
                        text: '您已超出您擁有的回饋幣',
                        type: 'error',
                ...
                if(this.authSellPrice >= this.maxSellPrice && this.maxSellPrice > 1) {
                    this.authSellPrice = 1 
                    let error = {
                        errorMsg: {
                            error: {
                                message: '最高拍賣價格為系統拍賣價格的1.1倍'
                            }
                        }
                    }
                    this.$store.dispatch('openAlert', error)
                }
                return Number(this.authSellValue*100).toLocaleString()
            },
            calGold() {
                return Number(this.authSellValue*this.authSellPrice*100).toLocaleString()
            },
            gold() {
